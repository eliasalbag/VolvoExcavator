...
def main():
    ...
    sampling_period = 0.1 # 10 ms = 100 Hz
    ...
    while True:
        ...
        if now - last_time >= sampling_period:
            ...
            # get angle values
            fused = fuse_sensors(sensor_manager, IMU_joints, kalman_filter, now)
            """
            ...
            if now - time_start > 1:
                print(len(sensor_manager["Rot_01"].position))
                print(len(sensor_manager["Rot_20"].position))
                print(len(sensor_manager["Rot_33"].position))
                print(len(sensor_manager["IMU_XAxis_128"].signals["AngleXAxis_128"]))
                print(len(sensor_manager["IMU_XAxis_137"].signals["AngleXAxis_137"]))
                print(len(sensor_manager["IMU_XAxis_138"].signals["AngleXAxis_138"]))
                print(len(sensor_manager["IMU_XAxis_139"].signals["AngleXAxis_139"]))
